 ...
 */
package com.helger.flugesel.extract;
...
public final class HybridExtractor
{
  ...
  /**
   * Extract the structured invoice XML from a hybrid invoice.
   *
   * @param aSource
  ...
  public static byte [] extractInvoiceXml (@NonNull final IHybridSource aSource) throws IOException
  {
    ...
    return HybridDocument.withOpenDocument (aSource, aDoc -> {
  ...
  /**
   * Extract one named attachment by its embedded-file name.
   *
   * @param aSource
   *        the source. May not be <code>null</code>.
   * @param sName
